...
RSpec.describe Discounts do
  ...
  describe 'Buy One Get One Free Discount' do
    it 'applies no discount for single item' do
      expect(buy_one_get_one_free(product: product, quantity: 1)).to eq(0)
    end

    it 'applies one freebie for two items' do
      expect(buy_one_get_one_free(product: product, quantity: 2)).to eq(11.1)
    end

    it 'applies one freebie for three items' do
      expect(buy_one_get_one_free(product: product, quantity: 3)).to eq(11.1)
    end

    it 'applies two freebies for four items' do
      expect(buy_one_get_one_free(product: product, quantity: 4)).to eq(22.2)
    end
  end

  describe 'Bulk Purchase Discount' do
    it 'applies no discount for quantity below limit' do
      expect(bulk_purchase_discount(product: product, quantity: 5, discount_limit: 6,
                                    discounted_price: 10.1)).to eq(0)
    end

    it 'applies discounted price for quantity equal to limit' do
      expect(bulk_purchase_discount(product: product, quantity: 7, discount_limit: 7,
                                    discounted_price: 9.6)).to eq(10.5)
    end

    it 'applies discounted price for quantity above limit' do
      expect(bulk_purchase_discount(product: product, quantity: 15, discount_limit: 8,
  ...
  describe 'Volume Ratio Discount' do
    it 'applies no discount for quantity below limit' do
      expect(volume_ratio_discount(product: product, quantity: 5, discount_limit: 10,
                                   discounted_ratio: '3/5')).to eq(0)
    end

    it 'applies discounted price for quantity equal to limit' do
      expect(volume_ratio_discount(product: product, quantity: 10, discount_limit: 10,
                                   discounted_ratio: '2/3')).to eq(37.0)
    end

    it 'applies discounted price for quantity above limit' do
      expect(volume_ratio_discount(product: product, quantity: 15, discount_limit: 10,
